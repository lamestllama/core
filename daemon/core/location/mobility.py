...
from emane.events import EventService, TDMAScheduleEvent, TDMASchedule
...
class OldNs2ScriptedMobility(WayPointMobility):
    """
...
@total_ordering
class ExtWayPoint(WayPoint):
    """
    Enhanced WayPoint supporting movement, EMANE config, and TDMA events.
    """
    def __init__(
        self,
        time: float,
...
class Ns2ScriptedMobility(OldNs2ScriptedMobility):
    """
    Extends NS2 scripted mobility to support EMANE setconfig and settdma commands,
    ...
    def readscriptfile(self) -> None:
        ...
        for line in f:
            ...
            if not line.startswith("$n"):
                continue
            try:
                # handle mobility and custom commands
                if line.startswith("$ns_ at "):
                    ...
                    # config event
                        ...
                        # seg = cmd.split()
                        # nodenum = int(seg[0].split("(")[1].split(")")[0])
                        _, _, dict_str = cmd.split(None, 2)
                        actions = eval(dict_str)
                        wp = ExtWayPoint(
                            line_time,
                            node_id=self.map(nodenum),
                            coords=None,
                            speed=None,
                            action="config",
                            value=actions,
    ...
    def addinitial(self, nodenum: int, x: float, y: float, z: float) -> None:
        ...
        :return: nothing
        """
        wp = ExtWayPoint(0, nodenum, coords=(x, y, z), speed=0)
        self.initial[nodenum] = wp
    ...
    def addwaypoint(
        ...
        """
        Override: use ExtWayPoint for all waypoint entries.
        """
        coords = (x, y, z) if x is not None and y is not None else None
        wp = ExtWayPoint(time, node_id, coords, speed, action, value)
        heapq.heappush(self.queue, wp)

    def updatepoints(self, now: float) -> None:
        """
        Move items from self.queue to either self.points (for movement)
        or handle EMANE config/tdma events immediately.
        """
        while self.queue and self.queue[0].time <= now:
            wp = heapq.heappop(self.queue)
            if wp.action == "dest":
                # regular movement waypoint
                self.points[wp.node_id] = wp
            elif wp.action == "config":
                # dispatch EMANE config event
                self.apply_config_event(wp.node_id, wp.value)
            elif wp.action == "tdma":
    ...
    def build_nem_map(self, node_id, grpc_address="127.0.0.1:50051"):   
        ...
        mac_list = [self.net.ifaces[x].mac for x in self.net.ifaces]

        node = client.get_node(self.session.id, node_id)
        # Loop through interfaces to find the "ctrl0" interface first
            ...
            if interface.name == "ctrl0":
                ...
                for intf in node[1]:  
                    if (intf.nem_id != 0 and 
                        intf.nem_port != 0 and
                        intf.mac in mac_list):
                        nem_map[intf.nem_id] = (ctrl_ip, intf.nem_port)        
                break   
        client.close() 
        return nem_map
    ...
    def apply_config_event(self, node_id: int, config: dict) -> None:
        ...
        for nem_id in nem_map:
            ip, port = nem_map[nem_id]
            client = ControlPortClient(ip, port)
            manifest = client.getManifest()
            for comp_type, params in config.items():
                comp_type_up = comp_type.upper()

                # find the build_id for this component
                try:
                    build_id = next(
                        b for b, c, _ in manifest[nem_id]
                ...
                config = client.getConfiguration(build_id)
                #  config is { paramName: [(currentValueTuple, typeId), …], … }

                # build a map of only the params this component actually supports
                supported = { name: typeId for name, val in config.items()
                                        for (_, typeId) in val }

                # pack only the valid updates into the required tuple form
                update_tuple = []
    ...
    def apply_tdma_event(self, node_id: int, updates: dict) -> None:
        ...
        schedule_file = node[0].emane_model_configs[(model, None)]["schedule"].value
        schedule = TDMASchedule(schedule_file)

        # retrieve schedule info and defaults
        info = schedule.info()
        defaults = schedule.defaults()

        # get the single NEM ID
        nem_id = next(iter(self.build_nem_map(node_id)))
        frames = info[nem_id]

        # apply the parameter patches in memory
        for frame_index, slots in frames.items():
            for slot_index, slot in slots.items():
                slot.update(updates)

        # build and publish a partial-update event for the NEM
        event = TDMAScheduleEvent(**defaults)
        for frame_index, slots in frames.items():
            for slot_index, slot in slots.items():
                event.append(
